...
app.post('/encrypt', (req, res) => {
  ...
  res.json({ ciphertext: cipherText, iv: nonce, authenticatedData: null, tag: aes.getAuthTag(), encrypted: encrypted });
});

app.post('/decrypt', (req, res) => {
  const { ciphertext, iv, tag, encrypted } = req.body as { ciphertext: Uint8Array, iv: Uint8Array, tag: Uint8Array, encrypted: Uint8Array };
  const sharedSecret = ml_kem768.decapsulate(ciphertext, secretKey);
  const key = sharedSecret.slice(0, 32);
  const aes = gcm(key, iv, tag);
...
app.post('/sign', (req, res) => {
  ...
  res.json({ signature: signature, verified: isValid });
});
