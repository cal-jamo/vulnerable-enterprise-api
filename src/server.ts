...
app.post('/encrypt', (req, res) => {
  ...
  res.json({ ciphertext: encrypted.toString('base64'), nonce: nonce.toString('base64'), encapsulatedKey: cipherText.toString('base64') });
});

app.post('/decrypt', (req, res) => {
  const { ciphertext, nonce, encapsulatedKey } = req.body as { ciphertext: string, nonce: string, encapsulatedKey: string };
  const sharedSecret = ml_kem768.decapsulate(Buffer.from(encapsulatedKey, 'base64'), secretKey);
  const key = sharedSecret.slice(0, 32);
  const aes = gcm(key, Buffer.from(nonce, 'base64'));
